    ...
    if (sharedTables.status === 'BETTING') {
        ...
        if (sharedTables.time <= 0) {
            ...
            io.to('baccarat').emit('sharedResults', { 
                room: 'baccarat', pCards: pC, bCards: bC, pScore: pS, bScore: bS, winner: bacWin, p3Drawn: p3Drawn, b3Drawn: b3Drawn
            });

            // 8 SECONDS to fully clear the table and restart timer
            setTimeout(() => {
